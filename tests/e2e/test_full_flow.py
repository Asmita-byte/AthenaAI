...
@pytest.mark.asyncio
async def test_upload_txt_file(client: AsyncClient, auth_headers: dict):
...
@pytest.mark.asyncio
async def test_full_upload_and_status_flow(client: AsyncClient, auth_headers: dict):
    ...
    status_response = await client.get(f"/status/{doc_id}", headers=auth_headers)
