...
class TextTool:
    ...
    name = "text_retriever"
    description = (
        "Retrieves relevant text passages from documents based on semantic and keyword search."
    )
    ...
    def run(self, query: str, document_ids: list[str] | None = None) -> list[dict]:
        ...
        text_results = [r for r in results if r.get("payload", {}).get("chunk_type") == "text"]
